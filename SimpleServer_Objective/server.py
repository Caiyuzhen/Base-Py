...
class WSGIServer():
	...
	def main():
		...
		# 2. 绑定本地信息 (# 调用等待函数)
		# server_socket.bind(("localhost", 3999)) # 绑定【域名】跟【端口】
		# 等待函数, 等待直到端口被释放
		def wait_for_port_release(port, timeout=60):
	...
	def run_forever(self):
		...
		while True:
			...
			self.new_socket, new_addr = server_socket.accept() # 🔥 用 self 来让整个类共享数据!! self.new_socket 为新的套接字, new_addr 为新的地址
	...
	def deal_with_request(self):
		...
		if ret:
			...
			try:
				file_path = self.document_root + file_name
